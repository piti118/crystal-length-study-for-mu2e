...
hitmap = root2array('carbonfiber.root','hitmap')
einfo = root2array('carbonfiber.root','eventinfo')
...
f = File('carbonfiger.hdf5','w')
f.create_dataset('result',data=result)
f.close()

# <codecell>

f = File('carbonfiber.hdf5','r')
tmp = f['result']
result = np.array(tmp)
f.close()
...
def doOneFit(E,range=(95.,110.),mean=104.,sigma=1.,n=20.,alpha=0.5,N=80000,
    ...
    while not good and itry<5:
        try:
            ...
            first =False
            fit,m = fit_binpoisson(eeg,E,maxcalls=100000,bins=100,
...
for i,angle in enumerate(angles):
    ...
    fit,m,fwhm = doOneFit(myE,range=(emin,emax),N=len(myE))
    title(str(angle)+' '+str(i))
    myresult[i]=(fit,m,fwhm)

# <codecell>

fwhm = np.zeros(18)
for i in range(18): fwhm[i]=myresult[i][2]
np.save('cf_fwhm.npy',fwhm)
...
myresult[1][0].show(myresult[1][1])
